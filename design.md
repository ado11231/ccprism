...
Other event types seen in real logs and **known-ignorable** for v1:
`attachment`, `permission-mode`, `mode`, `file-history-delta`,
`file-history-snapshot`. The parser keeps two skip buckets — known-ignorable
vs. genuinely unknown/malformed — and `doctor` only flags the latter.
`leafUuid` lives on `last-prompt` events (several per session; use the last).
